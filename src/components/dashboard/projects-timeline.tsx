...
function calculateLayout(tasks: Task[]): Map<string, ItemLayout> {
  ...
  sortedTasks.forEach(task => {
    ...
    // Update the level's end date. If it's a new level, push the end date.
                ...
                <div className="relative" style={{ width: `${containerWidth}px`, height: `${totalHeight}px` }}>
                    ...
                    <div className="absolute top-10 left-0 w-full h-full">
                        {months.flatMap(month => {
                            ...
                            const monthWidth = daysInMonth * dayWidth;
                            return (
                                <div className="absolute top-0 h-full border-r" style={{ left: getPosition(month), width: `${monthWidth}px` }}/>
                            )
                        })}
                    </div>
                                ...
                                return (
                                    <Tooltip key={task.id} delayDuration={100}>
                                        <TooltipTrigger asChild>
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    left: `${taskLeft}px`,
                                                    width: `${Math.max(taskWidth -2, 0)}px`, // -2 for margin
                                                    marginLeft: '1px',
                                        ...
                                        <TooltipContent>
                                            ...
                                            <p>Start: {task.startDate ? format(task.startDate, "MMM d, yyyy") : 'N/A'}</p>
                                            <p>Due: {task.dueDate ? format(task.dueDate, "MMM d, yyyy") : 'N/A'}</p>
