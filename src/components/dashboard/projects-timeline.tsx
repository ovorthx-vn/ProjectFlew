...
export function ProjectsTimeline({ projects }: ProjectsTimelineProps) {
  ...
  const { months, totalDays, startDate, totalHeight, dayWidth } = React.useMemo(() => {
    ...
    return { months, totalDays, startDate: start, totalHeight, dayWidth: 0 };
  }, [projects]);
  
  const getPosition = (date: Date, containerWidth: number) => {
    if (!date || containerWidth === 0) return 0;
    const dayWidth = containerWidth / totalDays;
    return differenceInCalendarDays(date, startDate) * dayWidth;
  }
  
  const getWidth = (start: Date, end: Date, containerWidth: number) => {
    if (!start || !end || containerWidth === 0) return 0;
    const dayWidth = containerWidth / totalDays;
            ...
            <div className="relative" style={{ height: `${totalHeight}px` }}>
                ...
                <div className="sticky top-0 z-10 flex bg-background/80 backdrop-blur-sm h-10 items-center border-b">
                    {months.map((month, index) => {
                        ...
                        const monthWidthPercentage = (daysInMonth / totalDays) * 100;
                    ...
                    <div key={project.id} style={{ top: `${projectTop}px`}} className="absolute w-full group">
                        <Tooltip delayDuration={100}>
                            ...
                            <div
                                ...
                                style={{
                                    left: `calc(${(differenceInCalendarDays(project.startDate, startDate) / totalDays) * 100}%)`,
                                    width: `calc(${((differenceInCalendarDays(project.dueDate, project.startDate) + 1) / totalDays) * 100}%)`,
